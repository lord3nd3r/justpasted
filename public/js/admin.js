...
(function(){
    ...
    root.innerHTML = `
        ...
        <section style="border:1px solid var(--border);border-radius:1rem;padding:1rem;margin-top:1.5rem;">
          ...
          <div style="display:flex;flex-direction:column;gap:0.6rem;max-width:700px">
            ...
            <fieldset style="border:1px solid var(--border);padding:0.6rem;border-radius:0.5rem">
              <legend>Mail settings</legend>
              <label>Host: <input id="cfg_mail_host" type="text" style="width:100%"></label>
              <label>Port: <input id="cfg_mail_port" type="number" style="width:120px"></label>
              <label><input type="checkbox" id="cfg_mail_secure"> Secure (SSL)</label>
              <label>Auth user: <input id="cfg_mail_user" type="text" style="width:100%"></label>
              <label>Auth pass: <input id="cfg_mail_pass" type="password" style="width:100%"></label>
              <label><input type="checkbox" id="cfg_mail_show_pass"> Show password</label>
              <label>From: <input id="cfg_mail_from" type="text" style="width:100%"></label>
            </fieldset>
            <div style="display:flex;gap:0.6rem">
              <button id="cfg_save" class="btn btn-success btn-sm">Save</button>
              <button id="cfg_test_smtp" class="btn btn-sm">Test SMTP</button>
  ...
  function attachHandlers(){
    ...
    function showUserDetail(userId){ userDetailEl.innerHTML='Loading user detail…'; Promise.all([ jfetch('/api/admin/user/'+encodeURIComponent(userId)+'/shares'), jfetch('/api/admin/users?q='+encodeURIComponent(userId)).catch(()=>[]) ]).then(([shares,userList])=>{ const user = Array.isArray(userList)? userList.find(u=>String(u.id)===String(userId)):null; const role = user? (user.role||'user') : 'user'; const banned = user? !!user.is_banned : false; let html=''; html+=`<div style="margin-bottom:0.6rem;"><strong>User ID:</strong> ${escapeHtml(userId)}</div>`; if(user){ html+=`<div style="margin-bottom:0.6rem;"><strong>Email:</strong> ${escapeHtml(user.email)}</div>`; html+=`<div style="margin-bottom:0.6rem;"><strong>Role:</strong> ${escapeHtml(role)}</div>`; html+=`<div style="margin-bottom:0.6rem;"><strong>Banned:</strong> ${banned? 'Yes':'No'}</div>`; } html+=`<div style="margin-bottom:0.8rem;"><label style="font-size:0.85rem;margin-right:0.4rem;">Set role:</label><select id="adminRoleSelect" style="padding:0.2rem 0.4rem;border-radius:0.4rem;border:1px solid var(--border);background:var(--bg-soft);color:var(--text-primary);"><option value="user"${role==='user'?' selected':''}>user</option><option value="mod"${role==='mod'?' selected':''}>mod</option><option value="admin"${role==='admin'?' selected':''}>admin</option></select><button id="adminRoleSaveBtn" class="btn btn-sm" style="margin-left:0.4rem;">Save</button></div><div style="margin-bottom:0.8rem;"><button id="adminBanToggleBtn" class="btn btn-sm ${banned? 'btn-success' : 'btn-danger'}">${banned? 'Unban user' : 'Ban user'}</button></div>`;
    ...
    function loadAdminConfig(){ const status = document.getElementById('cfg_status'); status.textContent='Loading…'; jfetch('/api/admin/config').then(cfg=>{ document.getElementById('cfg_email_verification').checked = !!cfg.EMAIL_VERIFICATION_ENABLED; document.getElementById('cfg_enable_admin_reg').checked = !!cfg.ENABLE_ADMIN_REGISTRATION; document.getElementById('cfg_quota_bytes').value = cfg.QUOTA_BYTES||''; if(cfg.MAIL){ document.getElementById('cfg_mail_host').value = cfg.MAIL.HOST||''; document.getElementById('cfg_mail_port').value = cfg.MAIL.PORT||''; document.getElementById('cfg_mail_secure').checked = !!cfg.MAIL.SECURE; document.getElementById('cfg_mail_user').value = cfg.MAIL.AUTH_USER||''; document.getElementById('cfg_mail_from').value = cfg.MAIL.FROM||''; } status.textContent=''; }).catch(e=>{ status.textContent='Error loading config'; }); }

    function saveAdminConfig(){ const status = document.getElementById('cfg_status'); status.textContent='Testing SMTP…'; const payload = { EMAIL_VERIFICATION_ENABLED: document.getElementById('cfg_email_verification').checked, ENABLE_ADMIN_REGISTRATION: document.getElementById('cfg_enable_admin_reg').checked, QUOTA_BYTES: Number(document.getElementById('cfg_quota_bytes').value)||undefined, MAIL: { HOST: document.getElementById('cfg_mail_host').value||undefined, PORT: Number(document.getElementById('cfg_mail_port').value)||undefined, SECURE: document.getElementById('cfg_mail_secure').checked, AUTH_USER: document.getElementById('cfg_mail_user').value||undefined, AUTH_PASS: document.getElementById('cfg_mail_pass').value||undefined, FROM: document.getElementById('cfg_mail_from').value||undefined } }; fetch('/api/admin/test-smtp',{ method:'POST', headers:{'Content-Type':'application/json'}, body: JSON.stringify({ MAIL: payload.MAIL }) }).then(r=>r.json()).then(resp=>{ if(resp&&resp.ok){ status.textContent='SMTP OK — saving…'; return fetch('/api/admin/config',{ method:'POST', headers:{'Content-Type':'application/json'}, body: JSON.stringify(payload) }).then(r=>r.json()); } const proceed = window.confirm('SMTP test failed: '+(resp&&resp.error?resp.error:'unknown')+"\nSave settings anyway?"); if(!proceed) throw new Error('aborted'); return fetch('/api/admin/config',{ method:'POST', headers:{'Content-Type':'application/json'}, body: JSON.stringify(Object.assign({},payload,{ force:true })) }).then(r=>r.json()); }).then(resp=>{ if(resp&&resp.ok){ status.textContent='Saved.'; setTimeout(()=>status.textContent='',2000); } else status.textContent = 'Save failed: '+(resp&&resp.error?resp.error:'unknown'); }).catch(err=>{ if(err.message==='aborted') return; status.textContent='Save error'; }); }
    ...
    document.getElementById('cfg_test_smtp').addEventListener('click', testSmtp);
    document.getElementById('cfg_mail_show_pass').addEventListener('change', e=>{ const pass = document.getElementById('cfg_mail_pass'); pass.type = e.target.checked ? 'text' : 'password'; });
